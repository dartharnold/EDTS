  ...
  def run(self):
    ...
    for y in self.args.systems:
      ...
      if extra_fuel is not None:
        ...
        yield Result(fuel = Fuel(initial = cur_fuel, final = cur_fuel + extra_fuel), refuel = Refuel(amount = extra_fuel, percent = self.ship.refuel_percent(extra_fuel)))
        cur_fuel += extra_fuel
        continue
      else:
        s = systems[y]
      if prev is not None:
        ...
        is_ok = True
        fmin, fmax = self.ship.fuel_weight_range(distance, allow_invalid=True)
        ...
        is_ok = (is_ok and fuel_cost <= self.ship.fsd.maxfuel and cur_fuel >= fuel_cost)
        yield Result(origin = Location(system = prev), destination = Location(system = s), distance = Lightyears(distance), cargo = self.args.cargo, ok = is_ok, is_long = is_long, fuel = Fuel(initial = cur_fuel, cost = fuel_cost, final = cur_fuel - fuel_cost, min = fmin, max = fmax))
        cur_fuel -= fuel_cost
      prev = s
